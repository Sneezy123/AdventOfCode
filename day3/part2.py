...
num_groups = len(rucksack_list) // 3
groups = []

for i in range(0, len(rucksack_list), 3):
    groups.append(rucksack_list[i:i + 3])
print(groups)
...
for group in groups:
    ...
    common_letters = list(set(s1) & set(s2) & set(s3))
    ...
    if common_letters[0].islower():
        ...
        priority = ord(common_letters[0]) - ord('a') + 1
    else:
        # Uppercase items have priorities 27 through 52
        priority = ord(common_letters[0]) - ord('A') + 27

    priorities.append(priority)

print(f"Sum: {sum(priorities)}")
